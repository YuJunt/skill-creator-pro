...
description: >
  专业级Agent Skill创建与优化工具。当用户讨论、询问、要求任何与"技能（Skill）"相关的任务时触发，包括但不限于：
  创建新技能、优化现有技能、改进技能、重构技能、升级技能、评审技能规范、技能审计、技能质量检查、测试技能、技能实测、E2E测试、技能问题排查、技能代码审查、技能架构设计、技能规范检查、技能深度审计、技能端到端实测、技能压力测试、技能成熟度评估。
  也适用于用户提到"skill""技能""agent skill""自定义技能""skill creator"等关键词并需要专业帮助的场景，以及用户说"帮我看看这个技能""这个技能有什么问题""技能还能怎么优化""技能现状如何""技能成熟度"等口语化表达。
  不适用于：简单prompt编写、非Skill格式的提示词优化、与技能创建无关的通用对话。
---
...
> 🔀 路由: {模式}
> ```
> 模式只能是以下4种之一：`新建技能` / `优化技能` / `深度评审` / `端到端测试`。
